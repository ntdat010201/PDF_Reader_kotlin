...
public class DefaultScrollHandle extends RelativeLayout implements ScrollHandle {
    ...
    private final static int HANDLE_LONG = 44;
    private final static int HANDLE_SHORT = 45;
    private final static int DEFAULT_TEXT_SIZE = 16;

    private float relativeHandlerMiddle = 0f;
    private CallbackTracking callbackTracking;
    ...
    public DefaultScrollHandle(Context context, CallbackTracking callbackTracking) {
        this(context, callbackTracking, false);
    }

    public DefaultScrollHandle(Context context, CallbackTracking callbackTracking, boolean inverted) {
        super(context);
    ...
    public void setupLayout(PDFView pdfView) {
        ...
        if (pdfView.isSwipeVertical()) {
            ...
            if (inverted) { // left
                ...
                background = ContextCompat.getDrawable(context, R.drawable.ic_bg_up_down);
            } else { // right
            ...
            if (inverted) { // top
                ...
                background = ContextCompat.getDrawable(context, R.drawable.ic_bg_page_by_page);
            } else { // bottom
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (pdfView.isSwipeVertical()) {
                    ...
                    float progress = relativeHandlerMiddle / (float) getHeight();
                    pdfView.setPositionOffset(progress, false);
                    if (progress > scrollHeight) {
                        callbackTracking.onScrollDown();
                    } else {
                        callbackTracking.onScrollUp();
                    }
                    scrollHeight = progress;
                } else {
